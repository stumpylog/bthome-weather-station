...
namespace bthome
{
  void Advertisement::doInit(std::string const& name, bool encrypt)
  {
    ...
    {
        this->m_deviceInfo = (this->m_encryptEnable << constants::BTHOME_DEVICE_INFO_SHIFTS::ENCRYPTED) | (constants::BTHOME_V2 << constants::BTHOME_DEVICE_INFO_SHIFTS::VERSION);
    ...
    const uint8_t* Advertisement::getPayload(void)
    {
      if(this->m_encryptEnable){
        ...
        mbedtls_ccm_encrypt_and_tag(&m_encryptCTX, textLen,
                                    nonce, constants::NONCE_LEN,
                                    0, 0,
                                    &this->m_data[m_sensorDataIdx],
                                    &ciphertext[0],
                                    encryptionTag, constants::MIC_LEN);
